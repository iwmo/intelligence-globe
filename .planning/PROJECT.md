...
| ViewportBbox interface in useAppStore.ts (not useViewportBbox.ts) | Hook imports store; defining type in hook would create circular import | ✓ Good — clean dependency direction |
| No mount-initialisation call in useViewportBbox | TDD test expects exactly 1 call after moveEnd; mount call would fire 2, failing VPC-01 | ✓ Good — test contract is authoritative |
| All 4 bbox params required for filter to activate (all() guard) | Partial bbox silently ignored — prevents edge cases with half-specified ranges | ✓ Good — correct fail-safe |
| BETWEEN on existing B-tree indexed lat/lon columns (no new indexes) | idx_aircraft_latlon_not_null already covers the hot path | ✓ Good — zero schema changes needed |
...
| Labels as separate LabelCollection (not embedded in PointPrimitiveCollection/BillboardCollection) | Cesium GPU architecture: LabelCollection is a distinct primitive type; cannot attach labels to existing primitive collections | ✓ Good — correct Cesium primitive model |
| `showEntityLabels` in `useSettingsStore` (not `useAppStore`) | Label visibility is a persisted preference, not transient runtime state; consistent with existing settings pattern | ✓ Good — correct store separation |
| `showEntityLabels` applied immediately in LOADED handler (not just Effect) | Effect alone cannot re-run when deps don't change after async worker response; LOADED handler fires on every data arrival | ✓ Good — fixes invisible-on-initial-load bug |
