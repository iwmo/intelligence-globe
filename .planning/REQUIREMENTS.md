...
- [ ] **GDELT-01**: System stores GDELT events in `gdelt_events` PostgreSQL table with UNIQUE `global_event_id`, `occurred_at`/`discovered_at` columns, lat/lon floats, `quad_class`, `goldstein_scale`, `num_mentions`, actor codes, `source_url`, and `source_is_stale` freshness field
