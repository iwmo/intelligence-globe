...
## Milestone: v9.0 — Entity Labels

**Shipped:** 2026-03-15
**Phases:** 1 (37) | **Plans:** 5 | **Files:** ~17 commits, 79 files changed

### What Was Built

- `showEntityLabels` boolean added to `useSettingsStore` with localStorage persistence (Zustand `persist`); Entity Labels checkbox in SettingsPanel Display section; opt-in, defaults to false
- Satellite labels: separate `LabelCollection` alongside `PointPrimitiveCollection`; `object_name` text in cyan (#00D4FF) monospace; `NearFarScalar(5e5, 1.2, 5e7, 0.0)` fades at global altitude matching PointPrimitive scale range
- Aircraft labels: `LabelCollection` alongside `BillboardCollection`; callsign/ICAO24 in orange (#FF8C00); military aircraft in red (#EF4444); ships (vessel_name/MMSI) in green (#22C55E) — all matching entity icon colors
- Gap closure (Plan 37-04): satellite labels not visible on initial load — `showEntityLabels` applied immediately in LOADED handler; aircraft minimum NearFarScalar scale floor 0.3 prevents Cesium culling at globe altitude (~2893km)
- Gap closure (Plan 37-05): military aircraft and ship labels stuck show:false on initial load — `.data` added to Effect 4 dependency array so visibility effect fires after Effect 2 populates the label maps

### What Worked

- **No backend changes** — all 10 requirements were pure frontend; label text already in each layer's React Query response (`object_name`, `callsign`, `vessel_name`); zero API work needed
- **LabelCollection as separate primitive** — correct Cesium architecture from day one; not fighting the GPU primitive model
- **LOADED handler + Effect pattern** — once the gap was diagnosed, the fix was clear: Effect alone can't re-run when deps don't change after async worker response; LOADED handler fires on every data arrival
- **Gap closure phases** — dedicating explicit plans (37-04, 37-05) to known gaps rather than trying to fix them in-line with new features kept the execution clean

### What Was Inefficient

- **Gap closure required 2 extra plans** — Plans 37-04 and 37-05 were not in the original 3-plan scope; the initial implementation left labels invisible in two important scenarios (initial load with persisted toggle, globe altitude). Deeper upfront analysis of Cesium's label/effect lifecycle would have caught these before shipping Plans 37-02/03
- **No milestone audit** — v9.0 proceeded without a dedicated audit pass; risk accepted in yolo mode

### Patterns Established

- **Zustand escape hatch in async callbacks** — `useSettingsStore.getState()` (not a selector) required inside async worker LOADED callbacks where selector-captured values go stale; same pattern as `useAppStore.getState()` in rAF loops
- **LabelCollection lifecycle mirrors BillboardCollection** — `labelsByKey` module-scope map cleared on unmount, same lifecycle as `billboardsByIcao24`; parallel primitive collections are the correct Cesium pattern for augmenting existing layers
- **Effect data dependency for visibility toggle** — when labels are created with `show: false` inside Effect 2 (data dep), Effect 4 (toggle dep) must also include the same data dep to fire after Effect 2 populates; otherwise Effect 4 runs before data and the toggle has nothing to operate on

### Key Lessons

1. **Cesium label show:false at creation is sticky** — labels created with `show: false` will not self-correct when a toggle effect fires if the data dep is missing; always include `.data` in the visibility effect's dep array when labels are built inside a data effect
2. **NearFarScalar minimum scale 0.0 culls labels** — Cesium stops rendering labels when `NearFarScalar` scale reaches 0.0; use a minimum floor (0.3+) if labels must remain visible at global altitude
3. **LabelCollection is not addable to PointPrimitiveCollection** — requires a separate primitive added to the scene; document this constraint at the start of any label feature to avoid surprises

### Cost Observations

- Sessions: 1 continuous session (2026-03-15)
- Model: balanced profile (sonnet-4.6)
- Notable: single-phase, 5-plan milestone (3 core + 2 gap closure) — gap closure plans are normal for Cesium rendering features where edge cases are hard to anticipate before seeing them in the browser
