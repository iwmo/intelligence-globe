...
describe('useAppStore — GDELT slices', () => {
  ...
  describe('layers.gdelt', () => {
    it('layers.gdelt defaults to false', () => {
      useAppStore.setState({ layers: { satellites: true, aircraft: true, militaryAircraft: false, ships: false, gpsJamming: false, streetTraffic: false, gdelt: false, earthquakes: false, fires: false, launches: false, installations: false } } as Parameters<typeof useAppStore.setState>[0]);
...
describe('useAppStore — visual engine and clean UI slices', () => {
  ...
  describe('detect overlay', () => {
    it('defaults off and toggles', () => {
      expect(useAppStore.getState().detectOverlayEnabled).toBe(false);
      useAppStore.getState().setDetectOverlayEnabled(true);
